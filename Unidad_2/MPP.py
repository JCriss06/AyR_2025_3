...
def contar_dedos(hand_landmarks, handedness):
    """
        Cuenta cuántos dedos están extendidos en una mano detectada.

        Parámetros:
        - hand_landmarks: objeto con las posiciones de los puntos clave de la mano.
        - handedness: cadena que indica si la mano es "Left" (izquierda) o "Right" (derecha).

        Retorna:
        - Un entero que representa el número de dedos extendidos.
    """
...
# Usa el modelo de MediaPipe Hands para detectar hasta 2 manos con una confianza mínima
...
with mp_hands.Hands(max_num_hands=2, min_detection_confidence=0.75) as manos:
    while True:
        ...
        # Voltea la imagen horizontalmente para que sea más intuitiva
        frame = cv2.flip(frame, 1)

        # Convierte la imagen a RGB para el procesamiento de MediaPipe
